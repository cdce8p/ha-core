...
@pytest.mark.skipif(
    sys.version_info >= (3, 15), reason="not yet available on Python 3.15"
)
...
@pytest.mark.skipif(sys.version_info < (3, 15), reason="still works on python 3.14")
async def test_memory_usage_py314(hass: HomeAssistant, tmp_path: Path) -> None:
    """Test raise an error on python3.14."""
    entry = MockConfigEntry(domain=DOMAIN)
    entry.add_to_hass(hass)
    ...
    with pytest.raises(
        HomeAssistantError,
        match="Memory profiling is not supported on Python 3.15. Please use Python 3.14.",
    ):
        await hass.services.async_call(
            DOMAIN, SERVICE_MEMORY, {CONF_SECONDS: 0.000001}, blocking=True
        )
