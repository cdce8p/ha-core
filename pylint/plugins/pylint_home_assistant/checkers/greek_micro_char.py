"""Checker for preferred coding of μ character."""
...
class HassEnforceGreekMicroCharChecker(BaseChecker):
    ...
    name = "home_assistant_enforce_greek_micro_char"
    priority = -1
    msgs = {
        "C7410": (
    ...
    def _check_const(self, node_const: nodes.Const | Any, node: nodes.NodeNG) -> bool:
        """Emit message if wrong char is used."""
        if (
            isinstance(node_const, nodes.Const)
            and isinstance(node_const.value, str)
            and "\u00b5" in node_const.value
        ):
            self.add_message("home-assistant-enforce-greek-micro-char", node=node)
            return True
        return False
    ...
    def _do_micro_check(
        ...
        if (
            ...
            and self._check_const(node.value, node)
        ):
            return
        ...
        if isinstance(target, nodes.AssignName) and isinstance(node.value, nodes.Dict):
            for _, subnode in node.value.items:
                ...
                for keyword in subnode.keywords:
                    if self._check_const(keyword.value, node):
                        return
